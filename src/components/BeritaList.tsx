...
export default function BeritaList({ news }: { news: News[] }) {
  ...
  const showFeatured =
    !!featured && (filter === 'all' || filter === featured.category) && matches(featured);
  const filtered = (filter === 'all' ? rest : rest.filter((n) => n.category === filter)).filter(
    matches
  );
  ...
  return (
    <>
      <PublicSearch
        ...
        resultCount={filtered.length + (showFeatured ? 1 : 0)}
