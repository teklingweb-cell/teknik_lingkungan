...
export type AdminListProps<T extends Row> = {
  ...
  /** Limits an admin screen to a stable subset of a shared table. */
...
export default function AdminList<T extends Row>({
  ...
  const scopedRows = rowFilter ? rows.filter(rowFilter) : rows;
  const view = scopedRows
  ...
  async function handleDelete() {
    ...
    // Drop the cached public page so the removed item disappears immediately.
    await revalidatePublic(table);
    await load();
  }
          ...
          <span className="card-count">
            {loading
              ...
              : q || (filterChips && chip !== filterChips.options[0]?.value)
                ? `${view.length} dari ${scopedRows.length} item`
                : `${scopedRows.length} item`}
          </span>
        </div>
