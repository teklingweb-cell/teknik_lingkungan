...
export type FieldDef =
  ...
  /** Pilihan kategori yang dimuat dari tabel mitra_kategori. */
  | {
      kind: 'mitra-category';
      name: string;
      /** Kolom teks lama yang ikut disimpan agar data sebelumnya kompatibel. */
      labelField: string;
      label: React.ReactNode;
      required?: boolean;
    }
  | { kind: 'gdrive'; name: string; label: string; folder?: string }
...
export type EntityFormProps = {
  ...
  /** Topbar heading, e.g. "Tambah Berita". */
   ...
   */
  slugFrom?: { field: string; table: 'news'; fallback?: string };
};
...
function Field({
  ...
  staff: string[];
  mitraCategories: { id: number; name: string }[];
}) {
  if (def.kind === 'row') {
    return (
      <div className="field-row">
        {def.fields.map((f) => (
          <Field
            ...
            set={set}
            staff={staff}
            mitraCategories={mitraCategories}
          />
        ))}
      </div>
    );
    ...
    return (
      <div className="field">
        ...
        <select
          ...
          value={current}
          onChange={(e) => {
            const category = mitraCategories.find((item) => String(item.id) === e.target.value);
            set(def.name, e.target.value);
            set(def.labelField, category?.name ?? '');
          }}
        >
          <option value="">Pilih kategori…</option>
          {mitraCategories.map((category) => (
            <option key={category.id} value={category.id}>
              {category.name}
            </option>
          ))}
        </select>
        {!mitraCategories.length && (
          <span className="form-hint">
            Belum ada kategori. Tambahkan melalui menu Kategori Mitra terlebih dahulu.
          </span>
        )}
      </div>
    );
...
export default function EntityForm({
  ...
  const [mitraCategories, setMitraCategories] = useState<{ id: number; name: string }[]>([]);
  ...
  const needsMitraCategories = useMemo(() => {
    const walk = (defs: FieldDef[]): boolean =>
      defs.some((d) => d.kind === 'row' ? walk(d.fields) : d.kind === 'mitra-category');
    return walk(fields);
  }, [fields]);
  ...
  useEffect(() => {
    if (!needsMitraCategories) return;
    ...
    (async () => {
      const { data } = await supabase
        ...
        .order('name', { ascending: true });
      if (!cancelled) {
        setMitraCategories((data ?? []) as { id: number; name: string }[]);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [needsMitraCategories, supabase]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (slugFrom && typeof payload.slug === 'string') {
      detail.push(`/berita/${payload.slug}`);
      if (previousSlug.current && previousSlug.current !== payload.slug) {
        detail.push(
          `/berita/${previousSlug.current}`
        );
      }
    }
