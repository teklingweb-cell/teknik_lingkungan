...
/**
 * Generated rather than hand-maintained, so every berita page is listed — the
 * old static sitemap.xml only covered the twelve top-level pages, leaving every
 * article invisible to crawlers.
 *
 * The limits are deliberate: a sitemap may hold 50,000 URLs, so 1,000 rows each
 * leaves plenty of headroom while keeping a small number of Supabase requests.
 */
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const now = new Date();

  // Via loadIndex so a missing `slug` column degrades to `select('*')` instead
  // of emptying the sitemap.
  const news = await loadIndex('news');
