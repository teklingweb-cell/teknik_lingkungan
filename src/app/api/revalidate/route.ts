...
const TARGETS: Record<string, { paths: string[]; routes?: string[] }> = {
  ...
  pencapaian: { paths: ['/akademik'] },
  kalender_akademik: { paths: ['/akademik'] },
  kurikulum: { paths: ['/akademik'] },
  mata_kuliah: { paths: ['/akademik'] },
  mitra: { paths: ['/mitra'] },
  mitra_kategori: { paths: ['/mitra'] },
  home_posters: { paths: ['/'] },
};
...
export async function POST(request: NextRequest) {
  ...
  if (Array.isArray(paths)) {
    for (const path of paths) {
      if (typeof path === 'string' && /^\/berita\/[a-z0-9-]+$/.test(path)) {
        revalidatePath(path);
        cleared.push(path);
      }
    }
