import type { Metadata } from 'next';
import { pageMetadata } from '@/lib/seo';
import './struktur.css';
import { supabasePublic } from '@/lib/supabase/public';
import type { Staff } from '@/lib/types';
import PageHero from '@/components/PageHero';
import OrgChart from '@/components/OrgChart';
import DosenDirectory from '@/components/DosenDirectory';
import StaffList from '@/components/StaffList';

// Read fresh on every request — see the note in (site)/page.tsx.
export const dynamic = 'force-dynamic';

export const metadata: Metadata = pageMetadata({
  title: 'Struktur Organisasi',
  description:
    'Bagan organisasi Program Studi Teknik Lingkungan Untan: ketua jurusan, koordinator program studi, dosen, dan staf administrasi.',
  path: '/struktur',
});

const LEGEND = [
  { color: '#1a2e1e', label: 'Ketua Jurusan' },
  { color: '#2d6a40', label: 'Koordinator Program Studi' },
  { color: '#2563eb', label: 'Dosen / Profesor' },
  { color: '#9333ea', label: 'Staf Administrasi' },
];

export default async function StrukturPage() {
  // The chart is the prodi's working structure, so only dosen and staf are
  // eligible — an alumnus carrying a stale org_level must not show up here.
  const [orgResult, lecturerResult, staffResult] = await Promise.all([
    supabasePublic
      .from('staff')
      .select('*')
      .not('org_level', 'is', null)
      .in('type', ['dosen', 'staf'])
      .order('org_level')
      .order('name')
      .limit(100),
    supabasePublic.from('staff').select('*').eq('type', 'dosen').limit(100),
    supabasePublic.from('staff').select('*').eq('type', 'staf').order('name').limit(100),
  ]);

  const people = (orgResult.data ?? []) as Staff[];
  const lecturers = (lecturerResult.data ?? []) as Staff[];
  const staff = (staffResult.data ?? []) as Staff[];

  return (
    <>
      <PageHero
        tag="Tentang Prodi"
        title="Struktur Organisasi"
        subtitle="Kepemimpinan dan susunan organisasi institusi kami yang berdedikasi pada kemajuan pendidikan."
        breadcrumb={[{ label: 'Tentang Prodi', href: '/profile' }, { label: 'Struktur Organisasi' }]}
      />

      <section style={{ padding: '60px 24px 20px', background: 'var(--bg)' }}>
        <div className="container">
          <div style={{ position: 'relative', width: '100%', height: 'auto', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
            <img src="/dosen-group.webp" alt="Dosen Teknik Lingkungan" style={{ width: '100%', height: 'auto', display: 'block' }} />
          </div>
        </div>
      </section>

      <section className="org-section">
        <div className="container">
          <div className="fade-up" style={{ marginBottom: 40, textAlign: 'center' }}>
            <div className="section-tag" style={{ justifyContent: 'center' }}>
              — Kepemimpinan
            </div>
            <h2 className="section-title" style={{ textAlign: 'center' }}>
              Bagan Organisasi
            </h2>
            <p
              style={{
                fontFamily: 'var(--font-body)',
                color: 'var(--text-muted)',
                fontSize: '0.9rem',
                marginTop: 8,
              }}
            >
              Klik kartu untuk melihat profil lengkap
            </p>
            <div className="mobile-scroll-hint" style={{ display: 'none' }}>
              Geser untuk melihat ↔
            </div>
          </div>

          <div className="org-legend fade-up">
            {LEGEND.map((l) => (
              <div key={l.label} className="org-legend-item">
                <div className="org-legend-dot" style={{ background: l.color }} /> {l.label}
              </div>
            ))}
          </div>

          <OrgChart people={[...people, ...lecturers, ...staff]} />
        </div>
      </section>

      <section className="dosen-section" aria-labelledby="dosen-heading">
        <div className="container">
          <div className="fade-up dosen-heading">
            <div className="section-tag">— Sumber Daya Akademik</div>
            <h2 className="section-title" id="dosen-heading">Dosen</h2>
            <p>Daftar dosen diurutkan berdasarkan tanggal lahir pada delapan digit pertama NIP.</p>
          </div>
          <DosenDirectory people={lecturers} />
        </div>
      </section>

      <section className="staff-section" aria-labelledby="staff-heading">
        <div className="container">
          <div className="fade-up dosen-heading">
            <div className="section-tag">— Dukungan Operasional</div>
            <h2 className="section-title" id="staff-heading">Staf</h2>
            <p>Tim administrasi dan laboratorium yang mendukung kegiatan Program Studi Teknik Lingkungan.</p>
          </div>
          <StaffList people={staff} />
        </div>
      </section>
    </>
  );
}
