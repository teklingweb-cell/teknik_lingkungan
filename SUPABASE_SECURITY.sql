...
alter table public.staff add column if not exists link_sinta text;

create table if not exists public.home_posters (
...
create table if not exists public.kalender_akademik (
  ...
  document_url  text,
  created_at    timestamptz default now(),
  constraint kalender_akademik_date_order check (end_date is null or end_date >= start_date)
);
...
-- Kategori tidak lagi berupa daftar yang tertanam di aplikasi. Baris mitra
-- lama tetap menyimpan teks `category`; FK ini menautkannya secara bertahap
-- tanpa menghapus atau mengubah data yang sudah ada.
create table if not exists public.mitra_kategori (
  id          bigserial primary key,
  name        text not null unique,
  color       text not null default '#4e8c5a'
                check (color ~ '^#[0-9A-Fa-f]{6}$'),
  created_at  timestamptz default now()
);
alter table public.mitra
  add column if not exists category_id bigint references public.mitra_kategori(id) on delete set null;

insert into public.mitra_kategori (name, color) values
  ('Industri', '#2563eb'),
  ('Pemerintah', '#d97706'),
  ('Akademik', '#16a34a'),
  ('Masyarakat', '#0891b2'),
...
update public.mitra as m
set category_id = k.id
from public.mitra_kategori as k
where m.category_id is null and lower(trim(m.category)) = lower(k.name);
...
alter table public.mitra_kategori   enable row level security;
...
alter table public.home_posters enable row level security;
...
begin
  for pol in
    ...
    where schemaname = 'public'
      and tablename in (
        'staff','penelitian','pencapaian','mitra','mitra_kategori','kalender_akademik','kurikulum','mata_kuliah','news','contact_messages'
      )
  loop
    execute format(
...
-- mitra categories
create policy "mitra_kategori_public_read" on public.mitra_kategori for select using (true);
create policy "mitra_kategori_admin_insert" on public.mitra_kategori for insert to authenticated with check (public.is_admin());
create policy "mitra_kategori_admin_update" on public.mitra_kategori for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy "mitra_kategori_admin_delete" on public.mitra_kategori for delete to authenticated using (public.is_admin());

-- academic calendar
create policy "kalender_akademik_public_read" on public.kalender_akademik for select using (true);
create policy "kalender_akademik_admin_insert" on public.kalender_akademik for insert to authenticated with check (public.is_admin());
create policy "kalender_akademik_admin_update" on public.kalender_akademik for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy "kalender_akademik_admin_delete" on public.kalender_akademik for delete to authenticated using (public.is_admin());

-- curriculum
create policy "kurikulum_public_read" on public.kurikulum for select using (true);
create policy "kurikulum_admin_insert" on public.kurikulum for insert to authenticated with check (public.is_admin());
create policy "kurikulum_admin_update" on public.kurikulum for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy "kurikulum_admin_delete" on public.kurikulum for delete to authenticated using (public.is_admin());

-- courses
create policy "mata_kuliah_public_read" on public.mata_kuliah for select using (true);
create policy "mata_kuliah_admin_insert" on public.mata_kuliah for insert to authenticated with check (public.is_admin());
create policy "mata_kuliah_admin_update" on public.mata_kuliah for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy "mata_kuliah_admin_delete" on public.mata_kuliah for delete to authenticated using (public.is_admin());
...
-- ─────────────────────────────────────────────────
-- 6. HOME POSTERS & IMAGE STORAGE
-- Public visitors may read poster rows and the bucket, while only allowlisted
-- admins may alter either. Image uploads are limited to 5 MB and are already
-- converted to WEBP by the admin interface before Storage receives them.
-- ─────────────────────────────────────────────────
do $$
declare pol record;
begin
  for pol in select policyname from pg_policies
    where schemaname = 'public' and tablename = 'home_posters'
  loop
    execute format('drop policy if exists %I on public.home_posters', pol.policyname);
  end loop;
end $$;

create policy "home_posters_public_read" on public.home_posters
  for select using (true);
create policy "home_posters_admin_insert" on public.home_posters
  for insert to authenticated with check (public.is_admin());
create policy "home_posters_admin_update" on public.home_posters
  for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy "home_posters_admin_delete" on public.home_posters
...
from pg_tables
where schemaname = 'public'
  and tablename in ('staff','penelitian','pencapaian','mitra','mitra_kategori','kalender_akademik','kurikulum','mata_kuliah','news','contact_messages','admins')
order by tablename;
...
from pg_policies
where schemaname = 'public'
  and tablename in ('staff','penelitian','pencapaian','mitra','mitra_kategori','kalender_akademik','kurikulum','mata_kuliah','news','contact_messages')
  and policyname not in (
    ...
    'mitra_kategori_public_read','mitra_kategori_admin_insert','mitra_kategori_admin_update','mitra_kategori_admin_delete',
    'kalender_akademik_public_read','kalender_akademik_admin_insert','kalender_akademik_admin_update','kalender_akademik_admin_delete',
    'kurikulum_public_read','kurikulum_admin_insert','kurikulum_admin_update','kurikulum_admin_delete',
    'mata_kuliah_public_read','mata_kuliah_admin_insert','mata_kuliah_admin_update','mata_kuliah_admin_delete',
