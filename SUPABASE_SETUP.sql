...
alter table penelitian add column if not exists keywords text;
alter table penelitian add column if not exists image_url text;
...
-- Kategori dikelola dari panel admin; kolom teks lama dipertahankan untuk
-- kompatibilitas data impor yang sudah ada.
create table if not exists mitra_kategori (
...
alter table mitra
  add column if not exists category_id bigint references mitra_kategori(id) on delete set null;
alter table mitra_kategori enable row level security;

insert into mitra_kategori (name, color) values
  ('Industri', '#2563eb'), ('Pemerintah', '#d97706'), ('Akademik', '#16a34a'),
  ('Masyarakat', '#0891b2'), ('Asosiasi', '#db2777'), ('Internasional', '#9333ea')
on conflict (name) do nothing;

update mitra as m
set category_id = k.id
from mitra_kategori as k
...
ALTER TABLE staff ADD COLUMN IF NOT EXISTS link_sinta    text;

-- ─── HOME POSTERS ─────────────────────────────────────────────────────────
-- Policies are intentionally created in SUPABASE_SECURITY.sql. Keeping RLS
-- enabled here makes a partial setup fail closed rather than expose content.
...
-- ─────────────────────────────────────────────────
-- TABLE: kalender_akademik, kurikulum, mata_kuliah
-- Managed from the Academic section in the admin panel.
-- Their secure RLS policies are created by SUPABASE_SECURITY.sql.
-- ─────────────────────────────────────────────────
