 ...
 */
/**
 * Potongan yang merupakan gelar, bukan nama orang.
 *
 * Diperiksa per kata setelah tanda baca dibuang, sehingga "S.T.", "M.Eng.Sc.",
 * "IPU", dan "ASEAN Eng" sama-sama dikenali. Sebelumnya "IPU" pada
 * "Ir. Kiki Prio Utomo, S.T., M.Sc., IPU" terbaca sebagai orang kedua.
 */
const DEGREE_TOKENS = new Set([
  's', 't', 'si', 'sc', 'pd', 'hut', 'kom', 'e', 'h', 'sos', 'ars', 'p',
  'ling', 'eng', 'm', 'dr', 'drs', 'ir', 'prof', 'ph', 'd', 'phd', 'ipu',
  'ipm', 'ipp', 'asean', 'rer', 'nat', 'ing', 'apt', 'ma', 'bsc', 'st', 'mt',
  'msi', 'msc', 'mling', 'meng', 'mengsc', 'mkom', 'mp', 'mm', 'mhum', 'ssi',
  'shut', 'spd', 'spdsi', 'skom', 'se', 'sh', 'ssos',
]);

function isDegreeFragment(piece: string): boolean {
  const tokens = piece
    .toLowerCase()
    .replace(/[^a-z ]/g, ' ')
    .split(/\s+/)
    .filter(Boolean);
  return tokens.length > 0 && tokens.every((t) => DEGREE_TOKENS.has(t));
}
...
export function parsePeople(raw: string | null | undefined): string[] {
  ...
  for (const piece of pieces) {
    if (names.length && isDegreeFragment(piece)) {
